...
function App(){
  // create state to watch for changes
  const [todos, setTodos] = useState([])
  const [newTask, setNewTask] = useState('')
  const [editingId, setEditingId] = useState(null)
  const [editingText, setEditingText] = useState('')

  // displays todos on page load
  useEffect(() => {
    fetchTodos()
  }, [])
  ...
  const fetchTodos = async () => {
    ...
    .from('todos')
    .select('*') //all the rows and all the colomns in a table
    .order('id', {ascending:true})
    if(error){
      console.log(error('Error fetching todos:', error))
    }else{
      setTodos(data)
    }
  }  
  ...
  const addTodo = async () => {
    ...
    .select()
    if(error){
      console.error(error('Error inserting fetching todos:', error))
        ...
        <ul style={{listStyleType: 'none', padding: 0}}>
          {todos.map((todo) => (
            <li key={todo.id} style={{marginBottom: '1rem', textAlign: 'center'}}>
              {editingId === todo.id ? (
                <input type="text" value={editingText} onChange={(e) => setEditingText(e.target.value)} onBlur={() => updateTodo(todo.id)}/>
              ) : (
                <div style={{display: 'flex', alignItems: 'center'}}>
                  <div>
                    <span
